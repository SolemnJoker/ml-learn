...
def D(img,num_h,reuse=False,alpha=0.01):
    '''
    img:输入图像
    n_units:判别器隐层数
    reuse:?
...
def G(noise_data,n_units,out_dim,reuse=False,alpha=0.01):
    '''
...
#generator
g_outputs = G(noise_data,g_units,img_size)

#discriminator
d_outputs_real = D(real_img,d_units)
d_outputs_fake = D(g_outputs,d_units,
reuse=True)
...
with tf.Session() as sess:
    with tf.device("/cpu:0"):
        ...
        for e in range(epochs):
            ...
            gen_samples = sess.run(G(noise_data, g_units, img_size, reuse=True),
